...
export function PaymentSuccessVerifier({
  ...
  useEffect(() => {
    ...
    async function verifyPayment() {
      try {
        ...
        setErrorMessage(
          result.error ??
          "Ödeme iyzico tarafından onaylanmadı."
        );
    ...
    return (
      <div className="relative mx-auto max-w-3xl rounded-2xl border border-white/14 bg-white/10 p-6 text-center shadow-2xl shadow-black/40 backdrop-blur-2xl sm:p-8">
        ...
        <p className="mt-5 text-base leading-8 text-slate-300">
          iyzico ödeme sonucu kontrol ediliyor.
  ...
  return (
    <div className="relative mx-auto max-w-3xl rounded-2xl border border-white/14 bg-white/10 p-6 text-center shadow-2xl shadow-black/40 backdrop-blur-2xl sm:p-8">
      ...
      <div className="mt-5 inline-flex rounded-full border border-yellow-500/30 bg-yellow-500/10 px-4 py-1 text-xs font-bold uppercase tracking-[0.18em] text-yellow-300">
        TEST MODE / SANDBOX
      </div>
      ...
      <p className="mt-5 text-base leading-8 text-slate-300">
        ...
        Fatura, sözleşme ve proje kickoff adımları için
